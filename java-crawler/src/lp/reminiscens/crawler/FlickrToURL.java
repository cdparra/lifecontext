...
public class FlickrToURL {
	...
	public String queryFlickr(String id) {
		...
		if (id == null) {
			qString = FlickrSearchQuery_url + FlickrQuery_per_page
					+ FlickrQuery_nojsoncallback + FlickrMin_Taken_Date
					+ minTakenDate + seconds + FlickrMax_Taken_Date
					+ maxTakenDate + seconds + FlickrExtraQuery
					+ FlickrExtraGeo + "%2C" + FlickrExtraTags + "%2C"
					+ FlickrExtraTakenDate + "%2C" + FlickrExtraUploadDate
					+ FlickrQuery_format + FlickrQuery_license
					+ FlickrQuery_tag + FlickrTag + FlickrQuery_key
					+ FlickrApiKey;
		} else {
			qString = FlickrGetLocationQuery_url + FlickrGetLocationQuery_id
					+ id + FlickrQuery_format + FlickrQuery_nojsoncallback
		...
		try {
			...
			for (int i = 0; i < JsonArray_photo.length(); i++) {
				...
				if (startdate.getHour().equalsIgnoreCase(startdate.getMinute())
						&& startdate.getMinute().equalsIgnoreCase(
								startdate.getSecond())
	...
	public void parseGeoJSON(String json, Media photo) {
		...
		try {
			...
			try {
				JSONObject Json_photo_data = Json_photo
						.getJSONObject("location");
				accuracy = Json_photo_data.getString("accuracy");
				photo.getLocation().setAccuracy(Integer.parseInt(accuracy));

				try {
					JSONObject Json_locality = Json_photo_data
				...
				try {
					...
					JSONObject Json_county = Json_photo_data
							.getJSONObject("county");
				...
				try {
					JSONObject Json_region = Json_photo_data
				...
				try {
					JSONObject Json_country = Json_photo_data
