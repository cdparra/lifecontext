...
public class CoordSearcher {
    ...
    public static void main(String[] args) throws MalformedURLException, UnsupportedEncodingException, IOException, JsonParseException {
    ...
    public String getReverseJsonByGoogle(String lat, String lon) throws MalformedURLException, UnsupportedEncodingException, IOException {

        URL url = new URL(reverseGeocodingString + lat + "," + lon + "&sensor=false");
        System.out.println(url);
    ...
    public void parseReverseGeoJson(String json, Event event) throws JsonParseException {

        try {
            JsonElement jelement = new JsonParser().parse(json);
            JsonObject Jsonresults = jelement.getAsJsonObject();
            JsonArray results = Jsonresults.getAsJsonArray("results");
            JsonArray components = results.get(0).getAsJsonObject().getAsJsonArray("address_components");
            ...
            try {
                for (int i = 0; i < components.size(); i++) {
                    types = components.get(i).getAsJsonObject().getAsJsonArray("types");

                    if (types.get(0).getAsString().equals("locality")) {
                        name = components.get(i).getAsJsonObject().get("long_name").getAsString();
                    }
                    
                    if (types.get(0).getAsString().equals("administrative_area_level_1")) {
                        region = components.get(i).getAsJsonObject().get("long_name").getAsString();
                    }

                    if (types.get(0).getAsString().equals("country")) {
                        country = components.get(i).getAsJsonObject().get("long_name").getAsString();
                    }
                }
    ...
    public void parseGeoJson(String json, Media photo) throws JsonParseException {
        try {
            JsonElement jelement = new JsonParser().parse(json);
            JsonObject Jsonresults = jelement.getAsJsonObject();
            JsonArray results = Jsonresults.getAsJsonArray("results");
            ...
            try {
                for (int i = 0; i < components.size(); i++) {
                    ...
                    if (types.get(0).getAsString().equals("administrative_area_level_3")) {
                        city = components.get(i).getAsJsonObject().get("long_name").getAsString();
                    }
    ...
    public void parseGeoJson(String json, Event event) throws JsonParseException {
        try {
            JsonElement jelement = new JsonParser().parse(json);
            JsonObject Jsonresults = jelement.getAsJsonObject();
            JsonArray results = Jsonresults.getAsJsonArray("results");
